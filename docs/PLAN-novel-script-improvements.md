...
### ✅ P1：人物形象稳定性（防漂移）——解决"同一角色两副脸"（已完成，commit `7c67d75`）
...
**验收清单（可感知）**：
- [x] 同角色"沈西风 / 沈 西 风 / 沈公子 / 沈西风大人"归并后人物表唯一（`TestMergeLightweightStoryCharacters`）
- [x] 不同角色（沈西风 vs 沈西楼）不误并（单测覆盖）
- [x] 生成样例 characters 含 personality/relations 且可解析（Unmarshal 测试 + 写作卡注入）
- [x] 既有 4 模式测试不回归（`go test ./internal/api -count=1` 全绿）

### ✅ P2：场景戏剧化 + 场景资产（解决"平铺直叙、同地点两张脸"）（已完成，commit `1015339`）
...
**验收清单（可感知）**：
- [x] h3_short 链路对缺 objective/turn/location 软告警落地（prompt 强制 + `validateLightweightStoryResponse` 告警，单测覆盖"告警不失败"）；"每场能写出谁要什么/遇到什么/结果怎么变"的生成物含量待 P6 手测
- [ ] 抽查 10 场，每场能写出"谁要什么 / 遇到什么 / 结果怎么变"（待 P6 手测）
- [ ] 同地点跨镜（如"客栈大堂 内 夜"出现 3 次），image_prompt 场景标签关键元素（陈设/光线）一致（待 P6 手测）

### ✅ P3：台词不丢 · 心理不堆 · 人不幽灵（后置质量检查层）（已完成，commit `1d14ab9`）
...
**验收清单（可感知）**：
- [x] 输入 5 句引号台词，报告能指出哪句没落位（`checkDialogueCoverage` 单测覆盖）
- [x] narration 占比超阈值被报告标注（`checkNarrationRatio` + 报告软/硬阈值双档）
- [x] 幽灵角色被报告列出（`checkGhostCharacters` + Markdown 报告列出具体名字）

### ✅ P4：自动修复（把"报错"变成"救回来"）（已完成，commit `534dc72` + `9a56b75`）
...
**验收清单（可感知）**：
- [x] 人为删掉某 scene 的 image_prompt → 自动补全一次成功（`TestRunLightweightStoryGenerationWithRetryFixesAndSucceeds`）
- [x] 台词缺失 → 重试后补齐（`TestRunLightweightStoryGenerationWithRetryDialogueLossTriggersRetry`，postQualityCheck 回调注入 P3 缺失清单）

### ✅ P5：质量报告落地展示（让"更好"看得见）（已完成，commit `0e3a595`）
...
**验收清单（可感知）**：
- [x] 一次 h3_short 生成结束后，进度日志出现三维度分数（P3 日志摘要 + P5 的 Markdown 任务结果）
- [ ] 修复前/后两次运行的分数可对比且修复后更高（待 P6 手测：同一输入连跑两次做人工对比）

### P6：端到端手测 + 文档收尾（进行中，代码侧 P0–P5 全部完成）

- [x] 文档更新为实施记录（本阶段 P1–P5 标记 ✅ + commit + 验收勾选，即本 commit）
- [x] 代码侧完整度：`go build ./...` + `go test ./internal/api -count=1` 全绿
- [ ] **端到端手测（待有真实 LLM 环境时执行）**：万字小说 → P0 转剧本 → h3_short 链路（P1–P5 全开）→ 质量报告
  - 手测脚本建议：同一输入连跑两次（第一次为对照），观察：
    1. 人物表无重复（沈西风/沈公子 归并唯一）
    2. 每场有 objective/conflict/turn
    3. 任务结果字段出现三维分数 Markdown
    4. 人为删 scene 的 image_prompt 后重跑 → 自动修复一次成功
- [ ] 已知限制复核（记录在案）：
